...
import "flowbite";
import {Badge, Button, Navbar, NavbarCollapse, NavbarLink } from "flowbite-react";
    ...
    <>
      ...
      >
        <Navbar className="fluid custom-nav lg:px-4 px-0 sm:px-0 py-6 bg-transparent dark:bg-transparent border-transparent dark:border-transparent">
          ...
          <MobileDrawer/>
          <NavbarCollapse className="xl:block hidden order-1 custom-navbar">
            {
              Links.map((item)=>{
                if(!item.isBadge){
                  return(
                    <NavbarLink as={Link} key={item.key} href={item.link} className={`text-[15px] ${pathname===item.link?"text-primary rounded-md dark:text-primary bg-lightprimary hover:!bg-lightprimary hover:!text-primary":"text-link dark:text-darklink  hover:!text-primary"}  font-medium`}>
                     {item.title}
                 </NavbarLink>
                  )
                }else{
                  return(
                    <NavbarLink key={item.key} as={Link} href={item.link} className={` flex items-center rounded-md group gap-2 ${pathname==="/frontend-pages/portfolio"?"text-primary dark:text-primary hover:!bg-lightprimary hover:!text-primary bg-lightprimary":"text-link dark:text-darklink "}`}>
